...
Usage:
  python totalseg_segmentation.py --input mri.nii.gz
  python totalseg_segmentation.py --input mri_dicom_dir --fast
"""
...
def collect_outputs(output_dir: Path) -> dict:
    seg_files = sorted(
...
def main():
    parser = argparse.ArgumentParser(description="TotalSegmentator MRI organ segmentation")
    parser.add_argument("--input", "-i", required=True,
                        help="MRI NIfTI file or DICOM folder/zip")
    ...
    output_dir = Path(args.output) if args.output else Path(tempfile.mkdtemp(prefix="totalseg_mri_"))
    ...
    print(f"[totalseg-mri] Running: {' '.join(cmd)}", file=sys.stderr)
